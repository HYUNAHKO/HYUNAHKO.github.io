...
let shader;
let textOverlay2;
let toonLevels = 1;  // 초기값을 1로 설정
let toonLevelsText;  // toonLevels 텍스트 오버레이를 위한 변수 추가
...
function setupKeyboardEvents() {
    document.addEventListener('keydown', (event) => {
        if (event.key == 'a') {
            ...
            updateText(toonLevelsText, "toon levels: " + toonLevels);
        }
    });
}
...
async function main() {
    try {
        ...
        shader.setVec3("light.specular", vec3.fromValues(0.3, 0.3, 0.3));
        ...
        shader.setVec3("u_viewPos", cameraPos);
        shader.setInt("toonLevels", toonLevels);  // 초기 toonLevels 설정

        setupText(canvas, "TOON SHADING", 1);
        textOverlay2 = setupText(canvas, "arcball mode: " + arcBallMode, 2);
        toonLevelsText = setupText(canvas, "toon levels: " + toonLevels, 3);  // toonLevelsText 변수에 저장
